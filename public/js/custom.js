...
function sendMail() {
    ...
    window.open(
        `mailto:admin@gmail.com?subject=Mail&body=Name: ${name} %0D%0A Email: ${email} %0D%0A Phone: ${phone} %0D%0A Message: ${message}`
    );
}
(function ($) {
    "use strict";
    ...
    $(".hdr-center-menu, .header-transparent-menu, #header-1").scrollToFixed({
        preFixed: function () {
            $("#header-1, .hdr-center-menu, .header-transparent-menu").addClass(
                "hdr-fixed"
            );
        },
        postFixed: function () {
            $(
                "#header-1, .hdr-center-menu, .header-transparent-menu"
            ).removeClass("hdr-fixed");
        },
    });
        ...
        $.ajax({
            ...
            error: function () {
                if (appointment_form === "sidebar") {
                    ...
                    $(".side-error").append(
                        '<div class="alert alert-danger" role="alert">Server Error Please try again after some time...</div>'
                    );
                } else {
                    $(".appointment-error .alert").remove();
                    $(".appointment-error").append(
                        '<div class="alert alert-danger" role="alert">Server Error Please try again after some time...</div>'
                    );
                }
            },
            success: function (response) {
                ...
                if (response.flag === "1") {
                    if (appointment_form === "sidebar") {
                        ...
                        $(".side-error .alert").remove();
                        $(".side-error").append(
                            '<div class="alert alert-danger" role="alert">Please enter/select ' +
                                response.error +
                                " ! ! !</div>"
                        );
                    } else {
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                if (data.flag === "1") {
                    ...
                    $(".contact-error .alert").remove();
                    $(".contact-error").append(
                        '<div class="alert alert-danger" role="alert">Please enter ' +
                            data.error +
                            " ! ! !</div>"
                    );
                } else {
                    ...
                    $(".contact-error .alert").remove();
                    $(".contact-error").append(
                        '<div class="alert alert-success" role="alert">Request Created Successfully.</div>'
                    );
                }
            },
        });
    ...
    $("body").on(
        "click",
        ".slider-appointment a, #appointment-now, .hdr-apointment",
        function () {
